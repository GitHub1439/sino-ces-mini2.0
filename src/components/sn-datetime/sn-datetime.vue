<template>
    <view class="sn-datetime-picker">
        ...
        <view class="sn-header" :class="{ 'sn-show': isShow }">
            <view class="sn-picker-header" @touchmove.stop.prevent="stop" catchtouchmove="stop">
                <view class="sn-btn-picker" :style="{ color: cancelColor }" hover-class="sn-opacity" :hover-stay-time="150" @click="hide">
                    取消
                </view>
                <view class="sn-btn-picker" :style="{ color: color }" hover-class="sn-opacity" :hover-stay-time="150" @click="btnFix">确定</view>
            </view>
            <view class="sn-picker-body">
                <picker-view :value="value" @change="change" class="sn-picker-view">
                    <picker-view-column v-if="!reset && type!=4">
                        <view class="sn-column-item" v-for="(item,index) in years" :key="index">
                            {{item}}<text class="sn-text">年</text>
                        </view>
                    </picker-view-column>
                    <picker-view-column v-if="!reset && type!=4">
                        <view class="sn-column-item" v-for="(item,index) in months" :key="index">
                            {{formatNum(item)}}<text class="sn-text">月</text>
                        </view>
                    </picker-view-column>
                    <picker-view-column v-if="!reset && (type==1 || type==2)">
                        <view class="sn-column-item" v-for="(item,index) in days" :key="index">
                            {{formatNum(item)}}<text class="sn-text">日</text>
                        </view>
                    </picker-view-column>
                    <picker-view-column v-if="!reset && (type==1 || type==4)">
                        <view class="sn-column-item" v-for="(item,index) in hours" :key="index">
                            {{formatNum(item)}}<text class="sn-text">时</text>
                        </view>
                    </picker-view-column>
                    <picker-view-column v-if="!reset && (type==1 || type==4)">
                        <view class="sn-column-item" v-for="(item,index) in minutes" :key="index">
                            {{formatNum(item)}}<text class="sn-text">分</text>
                        </view>
                    </picker-view-column>
                </picker-view>
            </view>
